...
export default function RatePlanOccupancySelect({ rateOccupancy, availableSpaces, onChange }) {
  console.log(rateOccupancy, availableSpaces)
  const isDecreaseDisabled = rateOccupancy <= 0;
  const isIncreaseDisabled = availableSpaces <= 0;

  const handleOccupancyDecrease = useCallback(() => onChange(--rateOccupancy), [rateOccupancy, onChange]);
  const handleOccupancyIncrease = useCallback(() => onChange(++rateOccupancy), [rateOccupancy, onChange]);
